...
df = pd.read_csv('data/train2.csv').astype(str)
...
features = [c for c in df.columns.values if c not in ['id', 'text', 'country']]
numeric_features = [c for c in df.columns.values if c not in [
    'id', 'text', 'country', 'processed']]
target = 'country'
...
pickle.dump(clf, open('model/model.h5', 'wb'))
